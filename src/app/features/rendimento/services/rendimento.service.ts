...
export class RendimentoService {
  ...
  getReturns(id: string, mode: 'simple' | 'cagr'): Observable<ReturnsResponse> {
    return this.http.get<ApiResponse<ReturnsResponse>>(`${this.base}/${id}/returns?mode=${mode}`).pipe(unwrapData());
  }

  getAllocation(id: string): Observable<AllocationItem[]> {
    return this.http.get<ApiResponse<AllocationItem[]>>(`${this.base}/${id}/allocation`).pipe(unwrapList());
  }
}
